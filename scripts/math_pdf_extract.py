...
内嵌于 shijuan-paiban skill，替代对外部 math-reference-read skill 的依赖。
CLI 接口与原脚本保持一致，SKILL.md 工作流 1-B 可直接调用：
  python math_pdf_extract.py <pdf> --output-dir ./math-output --language ch
"""
...
def main():
    ...
    p.add_argument("--output-dir", default="./math-output")
    ...
    # ── 图片 ──
    # mineru-open-sdk 不同版本返回形态不同：list of objects 或 {name: bytes} dict
    ...
    if imgs:
        ...
        if isinstance(imgs, dict):
            ...
            # 形态: [obj.name/.filename, obj.data/.content]
            for im in imgs:
