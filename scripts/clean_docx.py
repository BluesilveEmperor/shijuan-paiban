...
规则编号对照:
    ...
    1.19 浮动图片转嵌入式（执行前先通过 record_original_image_types 记录原始类型）
    (v3.5) record_original_image_types 预扫描记录 inline/anchor 类型，支撑双轨分流
"""
...
def record_original_image_types(doc, output_dir, logger):
    """预扫描：在浮动图片转换前，记录每张图片的原始类型（inline/anchor）。

    v3.5 双轨分流的基础：inline 图片由代码确定处理，anchor 图片由 AI 介入。
    本函数在 rule_1_19 转换浮动图片之前执行，将原始类型信息写入临时 JSON 文件，
    供 extract_images.py 读取后合并到 image_manifest.json 的 original_type 字段。

    Args:
        doc: python-docx Document 对象
        output_dir: 输出目录（用于保存 _original_image_types.json）
        logger: 日志记录器

    Returns:
        dict: {relationship_id: "inline" | "anchor" | "vml" | "unknown"}
    """
    ...
    for root in roots:
        for drawing in root.findall(f'.//{qn("w:drawing")}'):
            ...
            if rid in original_types:
                continue  # 同一 rId 可能出现在多处，只记录第一次
            ...
            elif drawing.find(f'.//{qn("v:imagedata")}') is not None:
                original_types[rid] = 'vml'
            else:
                original_types[rid] = 'unknown'

    # 保存到临时文件
    tmp_path = os.path.join(output_dir, '_original_image_types.json')
    os.makedirs(output_dir, exist_ok=True)
    with open(tmp_path, 'w', encoding='utf-8') as f:
        _json.dump(original_types, f, ensure_ascii=False, indent=2)

    inline_count = sum(1 for v in original_types.values() if v == 'inline')
    anchor_count = sum(1 for v in original_types.values() if v == 'anchor')
    logger.info(f'[原始类型记录] 共 {len(original_types)} 张图片: inline={inline_count}, anchor={anchor_count}, 已保存至 {tmp_path}')
...
def clean_docx(input_path, output_path, log_path=None, rules=None, pending_output_dir=None):
    ...
    try:
        ...
        # v3.5: 在转换浮动图片前记录原始类型（双轨分流基础）
        output_dir = os.path.dirname(output_path) or '.'
        record_original_image_types(doc, output_dir, logger)
        if should_run('1.19'):
