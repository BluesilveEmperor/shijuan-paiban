...
3. **结果检查**
   - 确认脚本退出码为 0
   - 确认 `{工作目录}/排版文档/final_exam.docx` 已生成且文件大小 > 0
...
```json
{
  ...
  "output_file": "{工作目录}/排版文档/final_exam.docx",
...
## Call Format
```bash
python scripts/typeset_exam.py --json {工作目录}/试卷数据/final_exam.json --template assets/template.dotx --images {工作目录}/清洗产物/images/ --output {工作目录}/排版文档/final_exam.docx
```

## Verification
- `{工作目录}/排版文档/final_exam.docx` 必须可被 Microsoft Word 或 WPS 正常打开
