...
def build_mapping(placeholders: list, images: list) -> dict:
    ...
    策略：
    1. 分离内容图片 (≥2KB) 和符号图片 (<2KB)
    2. 占位符按 question_stem 类型拆分为"有效占位符"和"泛型占位符"
    3. 有效占位符 + 内容图片 按题目顺序 / 段落顺序一一映射
    4. 泛型占位符如果与有效占位符同题，标记为冗余
    """
    # 分离内容图片和符号图片
    content_images = [img for img in images if img.get("_file_size", 9999) >= SYMBOL_SIZE_THRESHOLD]
    symbol_images = [img for img in images if img.get("_file_size", 9999) < SYMBOL_SIZE_THRESHOLD]

    # 按 paragraph_index 排序内容图片
    content_images.sort(key=lambda x: x.get("_paragraph_index", 9999))

    # 分类占位符：有效 (material / 有明确上下文) vs 泛型 (question_stem "阅读图文材料")
    valid_phs = []
    generic_phs = []
    for ph in placeholders:
        ...
        # 泛型占位符: question_stem 类型 + 上下文很短 + 原因笼统
        is_generic = (
    ...
    # 有效占位符按题目顺序排列 (section_id + question_number)
    ...
    for i, ph in enumerate(valid_phs):
        if i < len(content_images):
            ...
            # 尝试关键词匹配来提升 confidence
            ph_text = f"{ph.get('reason', '')} {ph.get('context_before', '')} {ph.get('context_after', '')}"
            img_context = img.get("_context", "")
            img_keywords = img.get("keywords", [])
            
            confidence = 0.7
            match_reason = f"段落顺序匹配（图在第{img.get('_paragraph_index', '?')}段）"
            
            # 关键词匹配增强
            kw_matches = [kw for kw in img_keywords if len(kw) >= 2 and kw in ph_text]
            if len(kw_matches) >= 2:
                confidence = 0.9
                match_reason = f"关键词匹配({len(kw_matches)}个): {', '.join(kw_matches[:4])}"
            elif len(kw_matches) == 1:
                confidence = 0.8
                match_reason = f"关键词部分匹配: {kw_matches[0]}"
    ...
    # 收集未使用的图片
    all_content_img_ids = {img.get("image_id") for img in content_images}
    unused_img_ids = sorted(all_content_img_ids - used_image_ids)
    unused_img_ids += sorted([img.get("image_id") for img in symbol_images])
    ...
    for img_id in unused_img_ids:
        ...
        if img:
            ...
            if size < SYMBOL_SIZE_THRESHOLD:
                ...
                warnings.append(f"{img_id}: 未找到匹配的占位符")

    if redundant_ph_ids:
         pass  # redundancy info captured in warnings
...
def main():
    ...
    print(f"  已映射: {mapped_count} | 未映射: {unmapped_count} | 未使用: {unused_count}")
