...
description: "Full pipeline for geography exam paper formatting: dual-track (code + AI) approach. Code handles inline images deterministically, AI only intervenes for floating/anchor images. Invoke when user provides a raw geography exam .docx file."
---

# 地理试卷排版 v3.5 — 代码+AI 双轨

六步解耦流水线，代码处理确定性逻辑（内嵌图片），AI 仅介入不确定性场景（浮动图片），最大化降低对模型智能度的依赖。
...
| **v3.5** | 代码 + AI 双轨 | 当前版本——代码处理 inline 图，AI 仅介入 anchor 图 |
...
| tag_placeholders_anchor | `.trae/skills/tag_placeholders_anchor/SKILL.md` | **仅对 anchor 浮动图**标注需插入图片的位置（inline 图由代码处理） |
| tag_images_anchor | `.trae/skills/tag_images_anchor/SKILL.md` | **仅对 anchor 浮动图**进行内容理解 |
| map_images | `.trae/skills/map_images/SKILL.md` | **双轨映射**：代码锁定 inline + AI 匹配 anchor |
