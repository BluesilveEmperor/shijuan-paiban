---
trigger: always_on
description: Geography exam paper formatting pipeline (v3.6). Dual-track code+AI approach.
---

# 地理试卷排版 AI 规则

> 完整规则见 AGENTS.md，本文件为 Windsurf 适配版。

## 项目概述

地理试卷排版系统 v3.6，六步解耦流水线，将 .docx 试卷转为排版精美 Word 文档。
核心策略：代码 + AI 双轨。代码处理确定性逻辑（inline 图），AI 仅介入不确定性场景（anchor 图）。v3.6 新增增量编辑和脚本优先策略。
...
- **Step5**：脚本优先（map_images.py 零 token），AI 仅处理未映射项，happy path 零 AI token
...
| `_source` | placeholders | `"anchor"`，占位符来源 |
| `position_hint` | anchor_descriptions.json | AI 对 anchor 图位置的分析 |
| `extent_w_cm` / `extent_h_cm` | image_manifest.json | 图片原始尺寸转厘米 |
