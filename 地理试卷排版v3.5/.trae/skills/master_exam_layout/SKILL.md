...
- **项目内置资源**：
  ...
  - `tag_placeholders_anchor` — Step3 anchor 图占位 Skill（替代原 tag_placeholders）
  - `tag_images_anchor` — Step4 anchor 图理解 Skill（替代原 tag_images）
  - `map_images` — Step5 双轨映射 Skill
...
```json
{
  ...
  "next_action": "执行 Step2: tag_structure（可并行启动 Step4: tag_images_anchor）"
}
```
...
### Step4: tag_images_anchor（anchor 图理解）— 与 Step2 并行

| 项目 | 内容 |
|------|------|
| **Skill** | `tag_images_anchor` |
...
```
请严格按照 tag_images_anchor 技能执行 anchor 图理解任务。
输入: {工作目录}/清洗产物/images/
...
### Step3: tag_placeholders_anchor（anchor 图占位）— 增量编辑模式

| 项目 | 内容 |
|------|------|
| **Skill** | `tag_placeholders_anchor` |
...
```
请严格按照 tag_placeholders_anchor 技能执行 anchor 图占位标注任务（增量编辑模式）。
操作流程:
...
### Step5: map_images（脚本优先 + AI 兜底）— v3.6 优化
...
```
请严格按照 map_images 技能执行 AI 兜底修正任务。
仅处理以下问题项：
