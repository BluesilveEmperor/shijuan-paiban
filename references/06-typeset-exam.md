# Step 6: Typeset Exam — 排版生成

> 本文档是 Geo Paper Format 流水线的 Step 6 详细规则。顶层入口见 `SKILL.md`。
> 本步骤并行生成版式一和版式二两个 Word 文档。
...
**关于材料中的图片标记**：`materials[].content` 字段可以内嵌 `{{image:ph_xxx}}` 图片占位标记。排版脚本会在材料文本中自动识别并插入对应图片，无需 `segments` 字段。此标记来自 Step3（tag-placeholders）。
