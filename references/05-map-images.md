# Step 5c: Map Images — AI 兜底修正（v3.6 脚本优先模式）

> 本文档是 Geo Paper Format 流水线的 Step 5c 详细规则。顶层入口见 `SKILL.md`。
>
> **重要**：本步骤**仅是 Step5 的 AI 兜底环节**，不是 Step5 的全貌。
> Step5 完整流程为：5a（脚本 `map_images.py` 优先映射，零 token）→ 5b（检查 validation 字段）→ 5c（AI 兜底，仅本文档描述的部分）。
> 只有当 5a/5b 发现未映射项或低置信度映射时，才触发本步骤。
